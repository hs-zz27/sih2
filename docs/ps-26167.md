...
* Codes and models including test and demonstration.

## Implementation Scope

The system shall support single optical/multispectral or SAR images, co-registered optical–SAR pairs, and bi-temporal pairs in GeoTIFF/TIFF or approved benchmark formats.
...
* change analysis;
* optical–SAR joint analysis; and
...
## Authoritative Sensor Note

The problem statement identifies the evaluation pairing as **Cartosat-2S optical + RISAT SAR**. It does not specify the exact RISAT mission/product (for example, EOS-04/RISAT-1A versus RISAT-2B/2BR1).

Therefore, the project should not assume a specific RISAT variant as the hidden SAC evaluation sensor unless that is independently confirmed by the organisers/ISRO/SAC. The implementation should remain sensor-configurable.

**Document purpose:** This file is the repository copy of the problem statement text supplied for SatQuery AI project traceability. It should be treated as the authoritative project requirement source when validating implementation coverage.
